...
public class MissileBase extends Entity implements IEntityAdditionalSpawnData {
    ...
    private BlockPos persistentBlockPos = BlockPos.;
    private IBlockState persistentBlock = Blocks.air;
    ...
    private void setMissileParams(IAmmo type, float x, float y, float z, float mx, float my, float mz) {
        ...
        {
            float temp = (float) Math.sqrt(mx * mx + my * my + mz * mz);
        ...
        if (ammoType.groundProximity() > 0) {
            while (groundDiff <= ammoType.groundProximity()) {
                ...
                if (!world.isAirBlock(new BlockPos(x, y - groundDiff, z))) {
                    this.onImpactWorld(new MovingObjectPosition(new Vec3d(x, y, z), EnumFacing.DOWN, new BlockPos(x, y, z))); //TODO correct MovingObjectPosition created? Test
                    impacted = true;
                    break;
                }
            }
